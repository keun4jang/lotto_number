...
def _fetch_rss_for_draw(draw_no: int, timeout: int = 10) -> str:
    """특정 회차를 지목한 검색어로 RSS를 조회 (백필용)."""
    import urllib.parse

    query = urllib.parse.quote(f"연금복권720+ {draw_no}회 당첨번호")
    url = _NEWS_RSS_SEARCH_TEMPLATE.format(query=query)
...
def fetch_pension_draw_by_number(
    draw_no: int,
    timeout: int = 10,
) -> Optional[PensionDraw]:
    """특정 회차를 지정해 검색어를 좁힌 뒤, 그 회차를 명시적으로 언급한 기사에서만
    당첨번호를 추출한다 (백필용).

    검색어 자체가 회차를 지정하므로, 기사 제목에 동일 회차 번호가 실제로
    포함된 경우에만 채택해 오탐을 방지한다.
    """
    rss_xml = _fetch_rss_for_draw(draw_no, timeout=timeout)
    titles = _parse_titles(rss_xml)
    ...
    for title, pubdate in titles:
        ...
        if not draw_match or not jo_num_match:
            continue
        if int(draw_match.group(1)) != draw_no:
            continue
        jo = int(jo_num_match.group(1))
        number = jo_num_match.group(2)
        if not (1 <= jo <= 5):
...
def backfill_pension_history(
    latest_draw_no: int,
    count: int = 20,
    delay_sec: float = 1.0,
) -> list[PensionDraw]:
    """latest_draw_no 직전 회차부터 count개의 과거 회차를 역순으로 백필한다.

    회차별 검색으로 오탐 가능성이 낮지만, 뉴스 검색 API에 부담을 주지
    않도록 요청 사이 delay_sec만큼 대기한다. 찾지 못한 회차는 건너뛴다.
    """
    results: list[PensionDraw] = []
    for draw_no in range(latest_draw_no - 1, latest_draw_no - 1 - count, -1):
        if draw_no < 1:
            break
        draw = fetch_pension_draw_by_number(draw_no)
        if draw is not None:
            results.append(draw)
